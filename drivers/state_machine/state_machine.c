...
enum {S_INIT,S_START,S_WAIT,S_SHORT,S_SHORT_SHORT,S_SHORT_LONG,S_LONG,S_LONG_SHORT,S_LONG_LONG};
enum { ACTION_NOP, ACTION_TS,ACTION_GO,ACTION_SHORT,ACTION_SHORT_SHORT,ACTION_SHORT_LONG,ACTION_LONG,ACTION_LONG_SHORT,ACTION_LONG_LONG };
...
struct state_machine example_3_state_machine[STATE_NUM][INPUT_NUM] = {
        {       /* State 0 ->init */
                { S_START, ACTION_TS  },      /* input SW_ON */
                { S_INIT, ACTION_NOP },      /* input SW_OFF */
                { S_INIT, ACTION_NOP }       /* input T_OUT */
        },
        {       /* State 1 -> start */
                { S_START, ACTION_NOP },  
                { S_SHORT, ACTION_NOP },  
	...
	{       /* State SHORT */
                { S_SHORT_SHORT, ACTION_TS},
                { S_SHORT, ACTION_NOP },
                { S_INIT, ACTION_SHORT }
        },
	{       /* State SHORT_SHORT */
                { S_SHORT_SHORT, ACTION_NOP },
                { S_INIT, ACTION_SHORT_SHORT },
                { S_SHORT_LONG, ACTION_TS }
        },
	{       /* State SHORT_LONG */
                { S_SHORT_LONG, ACTION_NOP },
                { S_INIT, ACTION_SHORT_LONG },
                { S_SHORT_LONG, ACTION_NOP }
        },

	{       /* State LONG */
                { S_LONG_SHORT, ACTION_TS },
                { S_LONG, ACTION_NOP },
                { S_INIT, ACTION_LONG }
        },

	 {       /* State LONG_SHORT */
                { S_LONG_SHORT, ACTION_NOP },
                { S_INIT, ACTION_LONG_SHORT },
                { S_LONG_LONG, ACTION_TS }
        },
	{       /* State LONG_LONG */
                { S_LONG_LONG, ACTION_NOP },
                { S_INIT, ACTION_LONG_LONG },
                { S_LONG_LONG, ACTION_NOP }
        },
...
int toy_state_machine_example_3(void)
{
	...
	int push;
	int put_led;
        unsigned long hard_delay_timer_count,not_push_time;


	pr_info("\n 토이 state machine 실험 3\n");
	set_bit_led(0x00);
        wait_queue_flag = WAIT_QUEUE_WAIT;
        hard_delay_timer_count = jiffies; /* Timer Start */
        not_push_time = jiffies; /* Timer Start */
        state = S_INIT;
	push=0;
	led=1;
...
loop:
	...
	if (((state == S_START) || (state==S_SHORT_SHORT) || (state==S_LONG_SHORT)) && (jiffies_to_msecs(jiffies - hard_delay_timer_count) >= 1000))
                input = T_OUT;
	else if(((state==S_SHORT)||(state==S_LONG) )&&(jiffies_to_msecs(jiffies- not_push_time) >= 1000) &&(push==1)){
		input = T_OUT;
		push=0;
	}
	...
	/* 2: Do action */
        switch (example_3_state_machine[state][input].action){ 
                        ...
                        while (led<=0xFF)
			{
				...
				led+=put_led;
				put_led*=2;
				usleep_range(10000,100001);
			}
                        break;
                case ACTION_SHORT_SHORT :
                        pr_info("<Short><Short>\n");
			led=0xFF;
			put_led=0x80;
                        while (led>1)
		...
		case ACTION_SHORT_LONG:
	        ...
	        case ACTION_LONG :
                        ...
                        set_bit_led(0x0F);
                        break;
		case ACTION_LONG_SHORT :
                        pr_info("<Long><Short>\n");
                        set_bit_led(0xFF);
                        break;

		case ACTION_LONG_LONG :
                        pr_info("<Long><Long>\n");
                        set_bit_led(0x00);
                        break;
                default :
	...
	if(((state==S_SHORT) ||(state==S_LONG) )&&(push==0)) {
		push=1;
		not_push_time = jiffies;
	}
